...
def _build_techniques(feature_df: pd.DataFrame, random_state: int, knn_neighbors: int) -> dict[str, Pipeline]:
    preprocessor, _, _ = _build_preprocessor(feature_df)
...
def main() -> int:
    ...
    print(f"  Filas con CO2 conocido: {valid_mask.sum():,}")

    if valid_mask.sum() < 100:
        print("Error: demasiado pocas filas con target conocido.")
        return 1

    # Aplica missing artificial sobre el target para simular imputacion.
