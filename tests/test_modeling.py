...
class TestBuildPipeline:
    def test_returns_four_values(self):
        ...
        result = build_pipeline(df, random_state=42, device="cpu")
        assert len(result) == 4

    def test_numeric_columns_detected(self):
        df = _sample_feature_df()
        _, numeric_cols, categorical_cols, _ = build_pipeline(df, random_state=42, device="cpu")
    ...
    def test_pipeline_can_fit_and_predict(self):
        ...
        pipeline, _, _, _ = build_pipeline(df, random_state=42, device="cpu")
...
class TestFitAndEvaluate:
    def test_returns_expected_keys(self):
        ...
        target = _sample_target()
        pipeline, _, _, _ = build_pipeline(df, random_state=42, device="cpu")
        scores = fit_and_evaluate(pipeline, df, target, random_state=42, cv_folds=3, n_jobs_cv=1)
    ...
    def test_cv_folds_respected(self):
        ...
        target = _sample_target()
        pipeline, _, _, _ = build_pipeline(df, random_state=42, device="cpu")
        scores = fit_and_evaluate(pipeline, df, target, random_state=42, cv_folds=4, n_jobs_cv=1)
